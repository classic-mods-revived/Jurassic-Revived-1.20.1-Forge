...
import net.cmr.jurassicrevived.entity.variant.GiganotosaurusVariant;
...
public class GiganotosaurusEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(GiganotosaurusEntity.class, EntityDataSerializers.INT);
    ...
    public GiganotosaurusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new SprintingMeleeAttackGoal(this, 1.25, false) {
            ...
            public boolean canUse() {
                return !GiganotosaurusEntity.this.isBaby() && super.canUse();
            }
            @Override
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.GIGANOTOSAURUS.get().create(pLevel);
        if (child instanceof GiganotosaurusEntity baby) {
            GiganotosaurusVariant randomVariant = Util.getRandom(GiganotosaurusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(GiganotosaurusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.giganotosaurus.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.giganotosaurus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.giganotosaurus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.giganotosaurus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.giganotosaurus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public GiganotosaurusVariant getVariant() {
        return GiganotosaurusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(GiganotosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof GiganotosaurusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        GiganotosaurusVariant variant = Util.getRandom(GiganotosaurusVariant.values(), this.random);
