...
import net.cmr.jurassicrevived.entity.variant.ShantungosaurusVariant;
...
public class ShantungosaurusEntity extends Animal implements GeoEntity {
    ...
    public ShantungosaurusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.SHANTUNGOSAURUS.get().create(pLevel);
        if (child instanceof ShantungosaurusEntity baby) {
            ShantungosaurusVariant randomVariant = Util.getRandom(ShantungosaurusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(ShantungosaurusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.shantungosaurus.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.shantungosaurus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.shantungosaurus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.shantungosaurus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.shantungosaurus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public ShantungosaurusVariant getVariant() {
        return ShantungosaurusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(ShantungosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof ShantungosaurusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        ShantungosaurusVariant variant = Util.getRandom(ShantungosaurusVariant.values(), this.random);
