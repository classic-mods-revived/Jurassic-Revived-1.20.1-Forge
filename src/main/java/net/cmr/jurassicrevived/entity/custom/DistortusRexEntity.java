...
import net.cmr.jurassicrevived.entity.variant.DistortusRexVariant;
...
public class DistortusRexEntity extends Animal implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(DistortusRexEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.DISTORTUS_REX.get().create(pLevel);
        if (child instanceof DistortusRexEntity baby) {
            DistortusRexVariant randomVariant = Util.getRandom(DistortusRexVariant.values(), this.random);
    ...
    public DistortusRexVariant getVariant() {
        return DistortusRexVariant.byId(this.getTypeVariant() & 255);
    }
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof DistortusRexEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        DistortusRexVariant variant = Util.getRandom(DistortusRexVariant.values(), this.random);
