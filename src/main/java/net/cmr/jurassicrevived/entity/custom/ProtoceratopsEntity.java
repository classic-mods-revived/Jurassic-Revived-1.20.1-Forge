...
public class ProtoceratopsEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(ProtoceratopsEntity.class, EntityDataSerializers.INT);
    ...
    public ProtoceratopsEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.PROTOCERATOPS.get().create(pLevel);
        if (child instanceof ProtoceratopsEntity baby) {
            ProtoceratopsVariant randomVariant = Util.getRandom(ProtoceratopsVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(ProtoceratopsEntity.this.isSprinting() ? RawAnimation.begin().then("anim.protoceratops.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.protoceratops.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.protoceratops.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.protoceratops.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.protoceratops.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public ProtoceratopsVariant getVariant() {
        return ProtoceratopsVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(ProtoceratopsVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof ProtoceratopsEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        ProtoceratopsVariant variant = Util.getRandom(ProtoceratopsVariant.values(), this.random);
