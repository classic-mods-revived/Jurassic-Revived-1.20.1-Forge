...
import net.cmr.jurassicrevived.entity.variant.DeinonychusVariant;
...
public class DeinonychusEntity extends Animal implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(DeinonychusEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.DEINONYCHUS.get().create(pLevel);
        if (child instanceof DeinonychusEntity baby) {
            DeinonychusVariant randomVariant = Util.getRandom(DeinonychusVariant.values(), this.random);
    ...
    public DeinonychusVariant getVariant() {
        return DeinonychusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(DeinonychusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof DeinonychusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        DeinonychusVariant variant = Util.getRandom(DeinonychusVariant.values(), this.random);
