...
import net.cmr.jurassicrevived.entity.variant.GuanlongVariant;
...
public class GuanlongEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(GuanlongEntity.class, EntityDataSerializers.INT);
    ...
    public GuanlongEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return GuanlongEntity.this.isBaby() && super.canUse();
            }
        });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.GUANLONG.get().create(pLevel);
        if (child instanceof GuanlongEntity baby) {
            GuanlongVariant randomVariant = Util.getRandom(GuanlongVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(GuanlongEntity.this.isSprinting() ? RawAnimation.begin().then("anim.guanlong.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.guanlong.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.guanlong.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.guanlong.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.guanlong.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public GuanlongVariant getVariant() {
        return GuanlongVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(GuanlongVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof GuanlongEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        GuanlongVariant variant = Util.getRandom(GuanlongVariant.values(), this.random);
