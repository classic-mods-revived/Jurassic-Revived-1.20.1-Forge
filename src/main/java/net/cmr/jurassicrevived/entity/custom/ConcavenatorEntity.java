...
import net.cmr.jurassicrevived.entity.variant.ConcavenatorVariant;
...
public class ConcavenatorEntity extends Animal implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(ConcavenatorEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.CONCAVENATOR.get().create(pLevel);
        if (child instanceof ConcavenatorEntity baby) {
    ...
    public ConcavenatorVariant getVariant() {
        return ConcavenatorVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(ConcavenatorVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof ConcavenatorEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        ConcavenatorVariant variant = Util.getRandom(ConcavenatorVariant.values(), this.random);
