...
public class BaryonyxEntity extends Animal implements GeoEntity {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.BARYONYX.get().create(pLevel);
    ...
    public BaryonyxVariant getVariant() {
        return BaryonyxVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(BaryonyxVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof BaryonyxEntity that)) return false;
