...
import net.cmr.jurassicrevived.entity.variant.HerrerasaurusVariant;
...
public class HerrerasaurusEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(HerrerasaurusEntity.class, EntityDataSerializers.INT);
    ...
    public HerrerasaurusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.HERRERASAURUS.get().create(pLevel);
        if (child instanceof HerrerasaurusEntity baby) {
            HerrerasaurusVariant randomVariant = Util.getRandom(HerrerasaurusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(HerrerasaurusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.herrerasaurus.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.herrerasaurus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.herrerasaurus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.herrerasaurus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.herrerasaurus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public HerrerasaurusVariant getVariant() {
        return HerrerasaurusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(HerrerasaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof HerrerasaurusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        HerrerasaurusVariant variant = Util.getRandom(HerrerasaurusVariant.values(), this.random);
