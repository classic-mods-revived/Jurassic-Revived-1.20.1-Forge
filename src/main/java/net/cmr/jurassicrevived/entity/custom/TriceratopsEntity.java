...
import net.cmr.jurassicrevived.entity.variant.TriceratopsVariant;
...
public class TriceratopsEntity extends Animal implements GeoEntity {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return TriceratopsEntity.this.isBaby() && super.canUse();
            }
        });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.TRICERATOPS.get().create(pLevel);
        if (child instanceof TriceratopsEntity baby) {
            TriceratopsVariant randomVariant = Util.getRandom(TriceratopsVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(TriceratopsEntity.this.isSprinting() ? RawAnimation.begin().then("anim.triceratops.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.triceratops.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.triceratops.idle", Animation.LoopType.LOOP));
        }));
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof TriceratopsEntity that)) return false;
