...
import net.cmr.jurassicrevived.entity.variant.RugopsVariant;
...
public class RugopsEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(RugopsEntity.class, EntityDataSerializers.INT);
    ...
    public RugopsEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return RugopsEntity.this.isBaby() && super.canUse();
            }
        });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.RUGOPS.get().create(pLevel);
        if (child instanceof RugopsEntity baby) {
            RugopsVariant randomVariant = Util.getRandom(RugopsVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(RugopsEntity.this.isSprinting() ? RawAnimation.begin().then("anim.rugops.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.rugops.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.rugops.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.rugops.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.rugops.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public RugopsVariant getVariant() {
        return RugopsVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(RugopsVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof RugopsEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        RugopsVariant variant = Util.getRandom(RugopsVariant.values(), this.random);
