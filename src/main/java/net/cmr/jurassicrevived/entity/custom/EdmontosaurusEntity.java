...
import net.cmr.jurassicrevived.entity.variant.EdmontosaurusVariant;
...
public class EdmontosaurusEntity extends Animal implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(EdmontosaurusEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.EDMONTOSAURUS.get().create(pLevel);
        if (child instanceof EdmontosaurusEntity baby) {
            EdmontosaurusVariant randomVariant = Util.getRandom(EdmontosaurusVariant.values(), this.random);
    ...
    public EdmontosaurusVariant getVariant() {
        return EdmontosaurusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(EdmontosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof EdmontosaurusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        EdmontosaurusVariant variant = Util.getRandom(EdmontosaurusVariant.values(), this.random);
