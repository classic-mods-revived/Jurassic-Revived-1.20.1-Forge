...
public class DiplodocusEntity extends Animal implements GeoEntity {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.DIPLODOCUS.get().create(pLevel);
        if (child instanceof DiplodocusEntity baby) {
            DiplodocusVariant randomVariant = Util.getRandom(DiplodocusVariant.values(), this.random);
    ...
    public DiplodocusVariant getVariant() {
        return DiplodocusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(DiplodocusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof DiplodocusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        DiplodocusVariant variant = Util.getRandom(DiplodocusVariant.values(), this.random);
