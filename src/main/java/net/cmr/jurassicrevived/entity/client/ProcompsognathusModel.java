...
public class ProcompsognathusModel extends GeoModel<ProcompsognathusEntity> {

    private static final Map<ProcompsognathusVariant, ResourceLocation> LOCATION_BY_VARIANT =
            Util.make(Maps.newEnumMap(ProcompsognathusVariant.class), map -> {
                map.put(ProcompsognathusVariant.MALE, ResourceLocation.fromNamespaceAndPath(JRMod.MOD_ID, "textures/entity/procompsognathus.png"));
                map.put(ProcompsognathusVariant.FEMALE, ResourceLocation.fromNamespaceAndPath(JRMod.MOD_ID, "textures/entity/procompsognathus_female.png"));
            });
    ...
    @Override
    public ResourceLocation getModelResource(ProcompsognathusEntity animatable) {
        return ResourceLocation.fromNamespaceAndPath(JRMod.MOD_ID, "geo/procompsognathus.geo.json");
    }

    @Override
    public ResourceLocation getTextureResource(ProcompsognathusEntity animatable) {
        return LOCATION_BY_VARIANT.get(animatable.getVariant());
    }

    @Override
    public ResourceLocation getAnimationResource(ProcompsognathusEntity animatable) {
        return ResourceLocation.fromNamespaceAndPath(JRMod.MOD_ID, "animations/procompsognathus.animation.json");
    }

    @Override
    public void setCustomAnimations(ProcompsognathusEntity entity, long id, AnimationState<ProcompsognathusEntity> state) {
        super.setCustomAnimations(entity, id, state);

        String[] tailBones = { "Tailpart1", "Tailpart2", "Tailpart3", "Tailpart4", "Tailpart5", "Tailend" };
        ...
        GeoBone head = (GeoBone) getAnimationProcessor().getBone("Chest2");
