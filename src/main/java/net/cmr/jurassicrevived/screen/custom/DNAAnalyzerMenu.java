...
public class DNAAnalyzerMenu extends AbstractContainerMenu {
    public final DNAAnalyzerBlockEntity blockEntity;
    public final Level level;
    public final ContainerData data;

    public DNAAnalyzerMenu(int containerId, Inventory inventory, FriendlyByteBuf data) {
        this(containerId, inventory, inventory.player.level().getBlockEntity(data.readBlockPos()), new SimpleContainerData(2));
    }

    public DNAAnalyzerMenu(int containerId, Inventory inventory, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.DNA_ANALYZER_MENU.get(), containerId);
        blockEntity = ((DNAAnalyzerBlockEntity) entity);
        ...
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 1, 80, 7) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.getItem() == ModItems.FROG_MATERIAL.get();
            }
            @Override
            public boolean mayPickup(Player playerIn) {
                return true; // Explicitly allow players (and JEI) to take items from this slot
        ...
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler, 2, 80, 63) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return false;
            }
