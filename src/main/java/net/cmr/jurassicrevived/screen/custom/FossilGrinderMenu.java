...
public class FossilGrinderMenu extends AbstractContainerMenu {
    public final FossilGrinderBlockEntity blockEntity;
    public final Level level;
    public final ContainerData data;

    public FossilGrinderMenu(int containerId, Inventory inventory, FriendlyByteBuf data) {
        this(containerId, inventory, inventory.player.level().getBlockEntity(data.readBlockPos()), new SimpleContainerData(2));
    }

    public FossilGrinderMenu(int containerId, Inventory inventory, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.FOSSIL_GRINDER_MENU.get(), containerId);
    ...
    public ItemStack quickMoveStack(Player playerIn, int pIndex) {
        ...
        if (pIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
            ...
            // Restrict to the input slot only (exclude all output slots).
            int teInputStart = TE_INVENTORY_FIRST_SLOT_INDEX;          // handler index 0
            int teInputEndExclusive = TE_INVENTORY_FIRST_SLOT_INDEX + 1;
